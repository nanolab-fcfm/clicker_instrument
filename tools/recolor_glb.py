...
script assigns a sensible material to each component mesh based on its reference
designator (R* = resistor, Q* = transistor, J* = connector/header), in place.

Usage:
    python tools/recolor_glb.py docs/assets/3d/clicker.glb
    python tools/recolor_glb.py in.glb out.glb     # write to a different file
...
PALETTE = {
    "R": ([0.09, 0.16, 0.42], 0.0, 0.5),   # blue metal-film resistor body
    "Q": ([0.04, 0.04, 0.05], 0.0, 0.6),   # black TO-92 transistor
    "J": ([0.05, 0.05, 0.06], 0.0, 0.7),   # black plastic header
}
